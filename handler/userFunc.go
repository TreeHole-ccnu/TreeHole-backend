...
func UserLogin(c *gin.Context) {
	...
	if model.ConfirmPhone(userLoginInfo.Phone) == 1 {
		SendUnauthorizedError(c, errno.ErrUserNotFound, nil, "User didn't exist ! ")
		return
	}

	//i = 1时，该用户还未注册、i = 2时，该用户输入密码错误、i = 3时，该用户输入密码正确
	i, err, level := model.ConfirmUserPhone(userLoginInfo.Phone, userLoginInfo.Password)
	if err != nil {
...
func UserRegister(c *gin.Context) {
	...
	if model.ConfirmPhone(data.Phone) == 0 {
		SendUnauthorizedError(c, errno.ErrUserAlreadyExist, nil, "the phone already exist! ")
		return
	}
